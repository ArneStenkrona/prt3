...
void main() {
    float z = texture(u_DepthBuffer, v_TexCoordinate).r;
    ...
    float depth = linearize_depth(texture(u_DepthBuffer, v_TexCoordinate).r);
    // Difference between depth of neighboring pixels and current.
    float depth_diff = 0.0;
    float d_right = linearize_depth(texture(u_DepthBuffer, v_TexCoordinate + vec2( w, 0.0)).r);
    float d_left  = linearize_depth(texture(u_DepthBuffer, v_TexCoordinate + vec2(-w, 0.0)).r);
    float d_down  = linearize_depth(texture(u_DepthBuffer, v_TexCoordinate + vec2(0.0,  h)).r);
    float d_up    = linearize_depth(texture(u_DepthBuffer, v_TexCoordinate + vec2(0.0, -h)).r);
    float eps = 0.0;
    ...
    vec3 normal = 2.0 * texture(u_NormalBuffer, v_TexCoordinate).rgb - 1.0;
    // Difference between depth of neighboring pixels and current.
    float normal_diff = 0.0;
    vec3 n_right = 2.0 * texture(u_NormalBuffer, v_TexCoordinate + vec2( w,  0.0)).rgb - 1.0;
    vec3 n_left  = 2.0 * texture(u_NormalBuffer, v_TexCoordinate + vec2(-w,  0.0)).rgb - 1.0;
    vec3 n_down  = 2.0 * texture(u_NormalBuffer, v_TexCoordinate + vec2(0.0,  h)).rgb - 1.0;
    vec3 n_up    = 2.0 * texture(u_NormalBuffer, v_TexCoordinate + vec2(0.0, -h)).rgb - 1.0;
